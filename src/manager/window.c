...
void reparent_child_under_frame(xcb_connection_t *const con, const xcb_window_t child, const xcb_window_t frame) {
    ...
    vcookies[1] = xcb_unmap_window_checked(con, child);

    // reparent child window under frame
    // TODO stop hardcoding position
    vcookies[2] = xcb_reparent_window_checked(con, child, frame, 4, 28);

    // map the frame
    vcookies[3] = xcb_map_window_checked(con, frame);

    // map the child
    vcookies[4] = xcb_map_window_checked(con, child);
...
void raise_window(xcb_connection_t *const con, const xcb_window_t win) {
    ...
    if ((err = xcb_request_check(con, c))) {
        LERR("When raising window 0x%08x: X error code: %u (%s)", win, err->error_code, xerrcode_to_str(err->error_code));
