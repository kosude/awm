...
int main(void) {
    ...
    LINFO("%d-bit window manager process started.", ((int) sizeof(void *)) * 8);
    ...
    if ((e = xcb_connection_has_error(con))) {
        LINFO("Error making X connection: %d", e);
        return -1;
    } else {
...
static void manage_window(xcb_window_t win, xcb_screen_t *scr) {
    ...
    if ((e = xcb_request_check(con, xcb_change_save_set_checked(con, XCB_SET_MODE_INSERT, win)))) {
        LERR("When adding window %u to save-set: X error code: %u", win, e->error_code);
    }
    ...
    if (e) {
        LERR("Error when getting client window geometry: %d", e->error_code);
        return;
    }
    ...
    if (e) {
        LERR("Error when creating frame window: %d", e->error_code);
...
static void reparent_window(xcb_window_t client, xcb_window_t frame) {
    ...
    for (uint32_t i = 0; i < sizeof(cookies) / sizeof(cookies[0]); i++) {
        ...
        if (err) {
            LERR("When reparenting window %u: X error code: %u", client, err->error_code);
            return;
        }
    }
}
